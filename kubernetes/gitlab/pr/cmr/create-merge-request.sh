...
#FIXME: 根据project_name通过search查询可能会查出多个，比如rise-inventory可能就查出了rise-inventory-management，然后取了第一个[0]

DEFAULT_PROJECTS=(
  "rise-price-b2c"
  "rise-consignment-b2c"
  "rise-tmall-integration"
  "rise-jingdong-integration"
  "rise-public-platform-integration"
  "rise-inventory"
  "rise-finance"
  "rise-product"
  "rise-cms"
...
DEFAULT_ASSIGN_NAME="Rise"
FEMALE_MEMBERS=("RafI" "small w")
...
function render_assignee_nick_name() {
  ...
  if [[ $WINDOWS_FLAG == 1 ]]; then
    ...
    if [[ "$assignee_name" == "RafI" ]]; then
      echo "Hello, RafI"
    elif [[ " ${FEMALE_MEMBERS[*]} " =~ ${assignee_name} ]]; then
      echo "Ms. ${assignee_name}"
    else
      echo "Mr. ${assignee_name}"
    fi
  fi
}
